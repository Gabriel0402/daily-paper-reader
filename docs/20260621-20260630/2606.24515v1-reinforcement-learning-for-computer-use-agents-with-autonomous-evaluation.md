...
tldr: 计算机使用智能体在图形界面中执行任务，但其强化学习因缺乏可扩展奖励信号而困难。本文提出利用视觉语言模型作为自主评估器，将评估反馈建模为噪声二元奖励通道，并推导噪声校正的奖励估计器用于PPO训练。在多个桌面环境实验中，该方法使任务成功率平均提升12.6个百分点，表明建模并校正评估噪声后，自主评估可用作实用的RL奖励信号。
source: arxiv
...
motivation: GUI智能体的强化学习因缺乏可扩展、机器可读的奖励信号而难以应用。
method: 使用视觉语言模型作为自主评估器，将其反馈建模为噪声二元奖励，推导噪声校正的PPO奖励估计器。
result: 在三个桌面环境上，校正评估奖励相比零样本和原始评估奖励分别平均提升12.6和5.1个百分点。
conclusion: 明确建模并校正评估噪声后，自主评估可作为GUI环境中RL的实用奖励信号。
---

## 摘要
计算机使用智能体通过直接在图形用户界面中进行感知和操作来执行高级用户目标。然而，由于开放式的桌面环境很少提供可扩展的、机器可读的奖励信号，强化学习在CUAs中仍然困难重重：任务成功通常依赖于视觉感知，难以通过手工设计的奖励函数或密集的人工标签来指定。我们提出了一种RL微调框架，该框架使用自主视觉-语言评估作为GUI智能体的可扩展监督信号。给定最终截图和原始指令，视觉-语言模型判断任务完成情况并提供终末反馈，而无需在策略优化过程中使用任务特定的启发式规则或人工标签。由于自主评估器并不完美，我们将其反馈建模为有噪声的二元奖励通道，并为近端策略优化推导出噪声校正的奖励估计器。在macOSWorld、Windows Agent Arena和OSWorld上的实验表明，校正后的评估器奖励优于零样本基线和原始评估器奖励，与零样本性能相比，成功率平均提高12.6个百分点，与原始评估器微调相比提高5.1个百分点。这些结果表明，当明确建模并校正评估器噪声时，自主评估可以作为GUI环境中RL的实用奖励信号。
