...
tags: ["query:vlm-rl"]
score: 7.0
evidence: 提出面向合作多智能体强化学习的知识共享框架
tldr: 针对多智能体强化学习DTDE框架中动作建议过度依赖教师、忽视师生兼容性导致性能下降的问题，提出共识驱动的通信与知识共享框架CCKS。通过对比学习构建基于局部观测的共识模型，智能体依据共识约束和共享知识智能采纳建议，平衡探索与学习。在Google Research Football和星际争霸II多智能体挑战中，CCKS显著提升合作效率与学习速度，且即插即用与现有算法兼容。
source: arxiv
...
motivation: 现有动作建议方法过度依赖教师指导，忽视师生兼容性，导致建议过多、稳定性差、性能下降。
method: 构建共识模型，利用对比学习从局部观测中学习；智能体基于共识约束和共享知识评分选择动作。
result: 在GRF和SMAC环境中，CCKS集成后显著提升合作效率、学习速度和整体性能，超越现有DTDE基线。
conclusion: CCKS即插即用，有效改善多智能体协作中的知识共享与探索-利用平衡。
---

## 摘要
在面向合作多智能体强化学习的分散式训练与分散式执行（DTDE）中，基于动作建议的知识共享促进了智能体之间的可解释且可扩展的合作。然而，当前的动作建议方法往往过于遵循教师的指导，而未评估师生兼容性，导致建议过度、稳定性欠佳以及性能下降。为克服这些挑战，本文提出了一种基于共识的通信与知识共享（CCKS）框架，该框架允许智能体基于共识衍生的约束采纳建议，并更智能地遵循教师指令。该机制使智能体能够在探索与向经验丰富的教师学习之间取得平衡，从而提升整体性能。关键在于共识模型的构建，为此我们提出在智能体训练阶段利用对比学习基于局部观测构建共识模型。在动作选择时，智能体根据共识与共享知识对动作进行评分和选择。CCKS设计为即插即用解决方案，可与现有DTDE算法无缝集成。在Google Research Football环境及复杂的星际争霸II多智能体挑战中进行的实验表明，与当前DTDE基线相比，集成CCKS显著提升了合作效率、学习速度与整体性能。代码见https://github.com/yuanxpy/CCKS。
