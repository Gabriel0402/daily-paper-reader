...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 通过三权分立架构在AI智能体中强制执行目标完整性
tldr: 针对AI智能体可能产生的目标偏离和有害行为，本文提出PEA（策略-执行-授权）架构，采用“三权分立”设计实现系统级安全。该架构通过密码学令牌将意图生成、授权与执行层隔离，并引入意图验证、谱系追踪及目标漂移检测等机制。研究通过形式化验证证明，该方法能将对齐从概率性的模型行为转变为结构化的系统约束，为自主智能体治理提供了稳固基础。
source: arxiv
...
motivation: 现有的RLHF等对齐方法仅提供概率性安全保证，难以防止智能体产生并执行其内部生成的有害目标。
method: 提出PEA架构，通过解耦意图、授权和执行层，并利用密码学锚点和语义门控实现结构化的目标完整性强制执行。
result: 建立了包含意图验证、谱系追踪和漂移检测的五大核心组件，并通过形式化验证证明了即使在模型受损时也能维持目标完整性。
conclusion: 该研究将智能体对齐从行为属性转变为系统级的结构约束，为构建安全可控的自主智能体提供了新的范式。
---

## 摘要
最近的证据表明，前沿 AI 系统可能表现出智能体失配（agentic misalignment），即使在没有明确用户请求的情况下，也会根据内部构建的目标生成并执行有害操作。现有的缓解方法，如人类反馈强化学习（RLHF）和宪法式提示（constitutional prompting），主要在模型层面运行，且仅提供概率性的安全保证。我们提出了策略-执行-授权（PEA）架构，这是一种在系统层面强制执行安全的“权力分离”设计。PEA 将意图生成、授权和执行解耦为独立且隔离的层，并通过加密约束的能力令牌（capability tokens）进行连接。我们提出了五项核心贡献：(C1) 用于确保能力-意图一致性的意图验证层（IVL）；(C2) 意图谱系追踪（ILT），通过加密锚点将所有可执行意图与原始用户请求绑定；(C3) 目标漂移检测，拒绝低于可配置阈值的语义偏离意图；(C4) 输出语义网关（OSG），利用结构化的 K × I × P 威胁计算（知识、影响力、策略）检测隐性胁迫；以及 (C5) 形式化验证框架，证明即使在对抗性模型受损的情况下也能维持目标完整性。通过将智能体对齐从行为属性转变为结构化强制的系统约束，PEA 为自主智能体的治理提供了坚实的基础。
