...
title_zh: 通过语言引导的语义线索实现多模态大语言模型在遮挡和小目标场景下的鲁棒定位
authors: "Beomchan Park, Seongho Kim, Hyunjun Kim, Sungjune Park, Yong Man Ro"
date: 2026-04-27
pdf: "https://arxiv.org/pdf/2604.24036v1"
tags: ["query:mmseg"]
score: 6.5
evidence: 利用多模态大模型进行定位和语义线索提取
tldr: 针对多模态大模型在拥挤场景中因遮挡和微小目标导致定位性能下降的问题，本文提出了一种利用语言引导语义线索（LGSC）的新方法。通过语义线索提取器（SCE）从视觉流中提取特征，并结合文本嵌入生成语言语义先验，将其重新整合进视觉管线以增强目标语义。实验证明该方法显著提升了模型在复杂拥挤场景下的定位鲁棒性。
source: arxiv
...
motivation: 现有的多模态大模型在处理拥挤场景时，容易受到遮挡和微小目标的干扰，导致视觉语义信息退化。
method: 提出语义线索提取器（SCE）并结合文本嵌入生成语言引导语义线索（LGSC），将其作为先验信息注入视觉管线以优化目标表征。
result: 实验结果表明，引入LGSC能有效提高多模态大模型在复杂拥挤场景中的目标定位准确率。
conclusion: 通过利用不受视觉退化影响的语言信息来引导视觉语义，可以显著增强模型应对遮挡和微小目标的鲁棒性。
---

## 摘要
尽管多模态大语言模型（MLLMs）在通用场景中增强了定位能力，但其在拥挤场景中的鲁棒性仍有待探索。拥挤场景带来了视觉挑战（即遮挡和小目标），这会损害物体语义并降低定位性能。相比之下，语言表达不受此类退化的影响，并保留了物体语义。基于这些观察，我们提出了一种利用语言引导语义线索（LGSCs）来克服这些限制的新方法。具体而言，我们的方法引入了一个语义线索提取器（SCE），从 MLLM 的视觉流水线中提取物体的语义线索。然后，我们利用相应的文本嵌入来引导这些线索，生成作为语言语义先验的 LGSCs。随后，这些线索被重新整合到原始视觉流水线中，以细化物体语义。大量的实验和分析表明，将 LGSCs 整合到 MLLM 中能有效提高拥挤场景下的定位准确率。
