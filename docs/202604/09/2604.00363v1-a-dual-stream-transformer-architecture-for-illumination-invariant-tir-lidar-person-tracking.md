...
tldr: 针对RGB-D跟踪在极端光照下失效的问题，本文提出一种基于热红外（TIR）和激光雷达深度（LiDAR-D）的双流Transformer架构。为解决多模态标注数据稀缺的挑战，研究采用顺序知识迁移策略和细粒度微分学习率策略，使模型在保留预训练特征提取能力的同时快速适应几何深度线索。实验证明该方法在全天候环境下表现优异，为机器人人体跟随提供了稳健且高效的解决方案。
selection_source: fresh_fetch
motivation: 传统的RGB-D跟踪在黑暗或强背光等极端光照条件下性能严重下降，限制了机器人在全天候环境下的应用。
method: 提出一种双流Transformer架构，结合顺序知识迁移和细粒度微分学习率策略，将热红外先验知识有效迁移至TIR-D领域。
result: "实验结果显示该追踪器在平均重叠度（AO）和成功率（SR）上分别达到0.700和58.7%，显著优于传统基准模型。"
conclusion: 该研究为全天候机器人人体跟随提供了一种实用且资源高效的TIR-LiDAR融合跟踪方案。
---
