...
tldr: 针对多步生成式策略在机器人控制中推理速度慢、难以满足高频闭环控制需求的问题，本文提出了一种原生单步生成式策略框架。该框架包含基于漂移的策略（DBP）和优化方法（DBPO），通过将迭代优化过程内化到模型参数中，在保持多峰分布建模能力的同时实现单步推理。实验证明，该方法在离线模仿学习和在线强化学习中表现优异，推理速度提升达100倍，并成功应用于105.2 Hz的高频实机控制。
source: arxiv
...
motivation: 现有的多步生成式策略因推理时需要多次迭代计算，导致计算开销巨大，难以满足机器人高频闭环控制和在线强化学习的需求。
method: 提出漂移策略（DBP），利用不动点漂移目标将迭代细化过程内化至模型参数，并结合DBPO框架实现稳定的在线强化学习更新。
result: DBP在保持或超越多步扩散策略性能的同时，推理速度提升了100倍，并在双臂机器人实机任务中实现了105.2 Hz的高频稳定控制。
conclusion: 该研究成功开发了一种兼具高效推理与强大建模能力的单步生成式策略，为高性能在线机器人控制提供了有效的解决方案。
---

## 摘要
尽管多步生成式策略通过建模多模态动作分布在机器人操作中表现出色，但它们在推理时需要多步迭代去噪。因此，生成每个动作需要数十到数百次网络函数评估（NFEs），这使得它们在进行高频闭环控制和在线强化学习（RL）时成本过高。为了解决这一局限性，我们提出了一种原生单步生成式策略的两阶段框架，将细化过程从推理阶段转移到训练阶段。首先，我们引入了基于漂移的策略（Drift-Based Policy, DBP），该策略利用不动点漂移目标将迭代细化过程内化到模型参数中，从而在设计上构建了一个单步生成式骨干网络，同时保留了多模态动作建模能力。其次，我们开发了基于漂移的策略优化（Drift-Based Policy Optimization, DBPO），这是一种在线强化学习框架，它为预训练的骨干网络配备了兼容的随机接口，从而在不牺牲单步部署特性的情况下实现稳定的同策略（on-policy）更新。大量实验证明了该框架在离线模仿学习、在线微调和真实世界控制场景中的有效性。DBP 在推理速度提升高达 100 倍的同时，性能达到或超过了多步扩散策略。在具有挑战性的操作基准测试中，它也始终优于现有的单步基准模型。此外，DBPO 在在线设置中实现了有效且稳定的策略改进。在真实世界双臂机器人上的实验证明了 105.2 Hz 的可靠高频控制。
