...
tags: ["query:vlm-rl"]
score: 6.5
evidence: 使用扩散模型进行轨迹生成的基于模型的强化学习
...
motivation: 现有的扩散世界模型引导机制要么忽略价值信息，要么因仅关注短期奖励而导致在短生成窗口内表现出短视性。
method: 提出AGD-MBRL，通过Sigmoid优势引导（SAG）和指数优势引导（EAG）来调整逆向扩散过程，使生成的轨迹向高优势值区域集中。
result: 在MuJoCo控制任务中，AGD-MBRL在采样效率和最终回报上显著优于PolyGRAD、在线奖励引导及PPO等基准模型，部分任务性能提升达2倍。
conclusion: 优势感知引导是解决扩散模型MBRL中短视问题的简单且有效的方案，能够通过重采样实现理论上的策略改进。
---

## 摘要
基于自回归世界模型的基于模型的强化学习（MBRL）面临复合误差问题，而扩散世界模型通过联合生成轨迹段来缓解这一问题。然而，现有的扩散引导方法要么仅基于策略（丢弃了价值信息），要么基于奖励（在扩散时界较短时会表现出短视性）。我们提出了用于 MBRL 的优势引导扩散模型（AGD-MBRL），该模型利用智能体的优势估计来引导反向扩散过程，使采样集中在预期能产生超出生成窗口更高长期回报的轨迹上。我们开发了两种引导方式：(i) Sigmoid 优势引导 (SAG) 和 (ii) 指数优势引导 (EAG)。我们证明，通过 SAG 或 EAG 引导的扩散模型允许我们对轨迹进行重加权采样，其权重随状态-动作优势的增加而增加，这在标准假设下意味着策略改进。此外，我们表明，与无引导的扩散模型相比，AGD-MBRL 生成的轨迹遵循改进后的策略（即具有更高的价值）。AGD 与 PolyGRAD 风格的架构无缝集成，通过引导状态分量同时保持动作生成受策略约束，且无需更改扩散训练目标。在 MuJoCo 控制任务（HalfCheetah、Hopper、Walker2D 和 Reacher）上，AGD-MBRL 的样本效率和最终回报优于 PolyGRAD、在线 Diffuser 风格的奖励引导以及无模型基准（PPO/TRPO），在某些情况下提升幅度达 2 倍。这些结果表明，优势感知引导是解决扩散模型 MBRL 中短时界短视问题的一种简单且有效的补救措施。
