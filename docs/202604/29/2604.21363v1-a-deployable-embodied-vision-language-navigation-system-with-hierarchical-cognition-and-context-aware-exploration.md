...
tldr: 本研究针对具身智能在嵌入式部署中面临的计算资源受限与推理能力不足的矛盾，提出了一种可部署的视觉语言导航（VLN）系统。该系统通过异步解耦感知、记忆与推理模块，构建认知记忆图并将其分解为子图以供大模型推理。同时，将探索问题建模为上下文感知的加权旅行维修员问题（WTRP），显著提升了导航效率。实验证明，该系统在资源受限的硬件上实现了高效、实时的导航表现。
source: arxiv
...
method: 采用异步解耦架构，结合认知记忆图分解技术与基于加权旅行维修员问题（WTRP）的上下文感知探索策略。
result: 在仿真和真实机器人实验中，该系统在保持资源受限硬件实时性的同时，显著提升了导航的成功率和效率。
conclusion: 该研究成功构建了一个兼顾高层推理与低层部署效率的VLN系统，为具身智能在真实场景下的落地提供了有效方案。
---
