...
tags: ["query:mmseg"]
score: 6.0
evidence: 使用视觉基础模型进行开放词汇交互检测
tldr: 本文针对开放词汇人机交互（HOI）检测中传统模型与视觉语言模型特征融合困难的问题，提出了SL-HOI框架。该方法完全基于DINOv3模型，利用其骨干网络进行精细定位，并利用其文本对齐的视觉头进行交互分类。通过将交互查询与图像特征共同输入视觉头，有效弥合了表示差异。SL-HOI在保持大部分参数冻结的情况下，在SWiG-HOI和HICO-DET基准上达到了SOTA性能，展示了流线型架构的优越性。
selection_source: fresh_fetch
motivation: 现有方法在结合传统HOI检测器与视觉语言模型时，面临跨模型表示差异导致的特征融合挑战。
method: 提出基于DINOv3的流线型框架，通过将交互查询与图像特征共同输入视觉头来弥合表示鸿沟，并仅添加少量可学习参数。
result: SL-HOI在SWiG-HOI和HICO-DET基准测试中均取得了最先进的性能，证明了该架构的有效性。
conclusion: 通过充分利用预训练模型的定位与分类能力并优化其交互机制，可以显著提升开放词汇HOI检测的效率与精度。
---
