...
tldr: 针对现有视觉语言导航（VLN）方法在动态、真实场景中泛化能力不足的问题，本文提出了DyGeoVLN框架。该框架通过跨分支特征融合将动态几何基础模型引入VLN，实现了显式的3D空间表示和视觉语义推理。此外，引入了无位姿自适应分辨率令牌剪枝策略，有效压缩长程导航中的冗余信息，降低了推理成本。实验证明该方法在多个基准测试中达到SOTA水平，且在真实环境中表现出极强的鲁棒性。
selection_source: carryover_cache
motivation: 现有的VLN方法大多基于静态场景假设，难以应对动态且复杂的真实世界环境。
method: 提出DyGeoVLN框架，通过融合动态几何基础模型增强3D空间感知，并利用自适应令牌剪枝策略优化长程导航效率。
result: 在多个主流基准测试上取得了最先进的性能，并在真实环境验证中展现了出色的鲁棒性。
conclusion: 将动态几何先验与高效的令牌管理相结合，是提升VLN智能体在复杂动态场景下导航能力的有效途径。
---
