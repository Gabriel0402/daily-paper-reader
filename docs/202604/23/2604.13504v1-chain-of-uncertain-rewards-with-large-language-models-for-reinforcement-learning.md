...
tldr: 针对强化学习中奖励函数设计繁琐且效率低下的问题，本文提出CoUR框架。该框架利用大语言模型自动化设计过程，通过代码不确定性量化和相似性选择机制减少冗余评估，并结合贝叶斯优化对解耦的奖励项进行高效搜索。实验证明，CoUR在IsaacGym和双向操作基准测试中显著提升了性能并降低了评估成本。
source: arxiv
...
motivation: 传统奖励函数设计依赖大量人工且存在冗余，且现有自动化方法往往忽略了中间决策点的不确定性。
method: 提出CoUR框架，结合大语言模型、代码不确定性量化、相似性选择机制以及针对解耦奖励项的贝叶斯优化。
result: 在IsaacGym和Bidexterous Manipulation共29个任务中，CoUR在性能表现和评估效率上均优于现有方法。
conclusion: CoUR通过整合LLM与不确定性量化，为高效、鲁棒的强化学习奖励函数自动化设计提供了一种新颖且有效的方案。
---

## 摘要
设计有效的奖励函数是强化学习（RL）的基石，但由于传统方法固有的低效和不一致性，这仍然是一个具有挑战性且劳动密集的过程。现有方法通常依赖于大量的辅助设计和评估步骤，这些步骤容易产生冗余，并忽略了中间决策点处的局部不确定性。为了应对这些挑战，我们提出了不确定性奖励链（Chain of Uncertain Rewards, CoUR），这是一个集成大语言模型（LLM）以简化 RL 环境中奖励函数设计与评估的新型框架。具体而言，CoUR 引入了代码不确定性量化以及一种相似性选择机制，该机制结合了文本和语义分析，以识别并重用最相关的奖励函数组件。通过减少冗余评估并对解耦的奖励项利用贝叶斯优化，CoUR 实现了对最优奖励反馈更高效、更稳健的搜索。我们在 IsaacGym 的 9 个原始环境以及 Bidexterous Manipulation 基准测试的所有 20 个任务中对 CoUR 进行了全面评估。实验结果表明，CoUR 不仅实现了更好的性能，还显著降低了奖励评估的成本。
