...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 基于大语言模型的智能体通过强化学习微调进行长程规划
tldr: 针对LLM智能体在长程任务中规划困难和强化学习奖励稀疏的问题，本文提出了一种子目标驱动的框架。该框架包含推理时的子目标分解规划机制，以及名为MiRA的强化学习训练方法，通过引入密集的里程碑式奖励信号提升性能。实验显示，该方法显著提升了Gemini和Gemma3等模型的成功率，在WebArena-Lite基准上超越了GPT-4o等闭源模型及现有SOTA开源模型。
selection_source: carryover_cache
motivation: LLM智能体在处理长程任务时容易丢失目标，且强化学习中稀疏的奖励信号导致模型难以学习有效的动作序列。
method: 提出了推理时的子目标分解规划框架，并开发了利用密集里程碑奖励信号进行强化学习训练的MiRA框架。
result: "MiRA将Gemma3-12B的成功率从6.4%大幅提升至43.0%，在WebArena-Lite基准上超越了GPT-4o和WebRL等模型。"
conclusion: 显式的推理规划与里程碑式奖励的结合能显著增强智能体的长程任务处理能力，为构建稳健的通用自主系统提供了有效路径。
---

## 摘要
基于大语言模型（LLM）的智能体已成为数字环境（包括移动界面、操作系统和网络浏览器）中强大的自主控制器。以网页导航为例，它需要处理动态内容和长动作序列，因此具有极大的挑战性。现有的基于 LLM 的智能体在长程规划方面主要面临两个方面的困难：在在线执行过程中，随着新信息的出现，它们往往会迷失方向，缺乏通往最终目标的清晰且自适应的路径；这一问题在强化学习（RL）微调过程中进一步加剧，稀疏且延迟的奖励使得智能体难以识别哪些动作导致了成功，从而阻碍了它们在长任务中保持连贯的推理。为了应对这些挑战，我们提出了两项贡献。首先，我们引入了一个智能体框架，该框架利用专有模型通过子目标分解进行在线规划。其次，我们提出了 MiRA（Milestoning your Reinforcement Learning Enhanced Agent），这是一个使用密集的、基于里程碑的奖励信号的强化学习训练框架。实时规划机制使 Gemini 等专有模型在 WebArena-Lite 基准测试中的成功率（SR）绝对值提升了约 10%。同时，将 MiRA 应用于开源的 Gemma3-12B 模型，使其成功率从 6.4% 提升至 43.0%。这一性能超过了 GPT-4-Turbo (17.6%) 和 GPT-4o (13.9%) 等专有系统，以及之前的开源模型最优水平 WebRL (38.4%)。总之，我们的研究结果表明，将显式的推理时规划与基于里程碑的奖励相结合，可以显著提升智能体的长程能力，为构建更稳健、更通用的自主系统铺平了道路。
