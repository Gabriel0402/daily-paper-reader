...
tags: ["query:gt-mai"]
score: 8.0
evidence: 多智能体系统协调与行为规范
tldr: 本研究针对多智能体系统中权限管理碎片化、难以维护的问题，提出了基于ALARA（尽可能低）原则的上下文工程方法。通过引入声明式的CAT（上下文-智能体-工具）数据层和命令行工具npcsh，实现了对智能体工具访问和上下文的最小化授权。该系统通过结构化解析确保行为变更的强制执行，而非仅依赖模型遵循指令。实验评估了22个本地模型在115项任务中的表现，证明了该方法在提升多智能体协作安全性与可维护性方面的有效性。
selection_source: carryover_cache
motivation: 现有多智能体框架缺乏统一且可扩展的机制来管理智能体权限和上下文，导致配置碎片化且难以协作维护。
method: 提出声明式的CAT数据层及npcsh工具，通过结构化文件定义并强制执行智能体的最小权限访问。
result: 在对22个本地模型进行的约2500次测试中，验证了该系统能有效控制智能体行为并刻画了不同规模模型的任务处理能力。
conclusion: 该研究为构建可移植、可组合且符合最小特权原则的多智能体系统提供了一种标准化的解决方案。
---
