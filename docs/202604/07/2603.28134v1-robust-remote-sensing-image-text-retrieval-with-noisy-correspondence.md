...
title_zh: 具有噪声对应关系的鲁棒遥感图文检索
authors: "Qiya Song, Yiqiang Xie, Yuan Sun, Renwei Dian, Xudong Kang"
date: 2026-03-30
pdf: "https://arxiv.org/pdf/2603.28134v1"
tags: ["query:mmseg"]
score: 6.0
evidence: 桥接遥感视觉与语言理解
tldr: 针对遥感图文检索（RSITR）中普遍存在的图文不匹配（噪声对应）问题，本文提出了RRSITR框架。该框架采用自步学习策略，模拟人类由易到难的认知模式，根据损失大小将样本分为清洁、模糊和噪声三类并动态分配权重。通过引入鲁棒三元组损失和多模态自步函数，该方法有效提升了模型在噪声数据下的检索性能，在多个基准数据集上达到了领先水平。
selection_source: fresh_fetch
motivation: 现有的遥感图文检索研究通常假设图文对完美对齐，但在实际应用中获取高质量对齐数据成本高昂且数据集中普遍存在噪声对应问题。
method: 提出一种基于自步学习的鲁棒检索范式，通过动态评估样本可靠性并结合可调节软边距的鲁棒三元组损失来处理不同程度的噪声数据。
result: 在三个主流遥感基准数据集上的实验证明，RRSITR在各种噪声比例下均显著优于现有最先进方法，展现了极强的鲁棒性。
conclusion: 本研究首次探讨并解决了遥感图文检索中的噪声对应挑战，为构建更具鲁棒性的跨模态遥感理解系统提供了新思路。
---

## 摘要
作为连接遥感视觉与语言理解的关键任务，遥感图文检索（RSITR）近年来引起了广泛的研究兴趣。然而，几乎所有的 RSITR 方法都隐含地假设图像-文本对是完美匹配的。在实践中，获取大量对齐良好的数据对通常极其昂贵，甚至不可行。此外，我们还注意到遥感数据集（例如 RSITMD）确实包含一些不准确或不匹配的图像文本描述。基于上述观察，我们揭示了 RSITR 中一个重要但尚未被触及的问题，即噪声对应（NC）。为了克服这些挑战，我们提出了一种新型的鲁棒遥感图文检索（RRSITR）范式，该范式设计了一种自步学习策略来模拟人类的认知学习模式，从而在具有 NC 的多模态数据中实现由易到难的学习。具体而言，我们首先根据每个样本对的损失大小将所有训练样本对分为三类，即干净样本对、模糊样本对和噪声样本对。然后，我们通过根据损失值给每个样本对分配权重，分别评估每个训练对的可靠性。此外，我们分别设计了一种新的多模态自步函数，以动态调节样本的训练顺序和权重，从而建立一个渐进式的学习过程。最后，针对噪声样本对，我们提出了一种鲁棒的三元组损失，根据语义相似度动态调整软间隔，从而增强对噪声的鲁棒性。在三个流行基准数据集上的广泛实验表明，所提出的 RRSITR 显著优于现有最先进的方法，尤其是在高噪声率下。代码可在以下网址获取：https://github.com/MSFLabX/RRSITR
