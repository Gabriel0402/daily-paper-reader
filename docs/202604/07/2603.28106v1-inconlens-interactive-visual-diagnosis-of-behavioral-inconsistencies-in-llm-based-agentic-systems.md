...
tags: ["query:gt-mai"]
score: 6.0
evidence: 诊断基于大语言模型的智能体系统中的行为不一致性
tldr: 针对LLM智能体在相同输入下表现出的行为不一致性，本文开发了可视化分析系统InconLens。该系统通过引入“信息节点”作为中间抽象，将冗长无序的执行日志转化为可对比的语义里程碑，支持开发者交互式地诊断多次运行间的行为分歧。研究通过案例分析和专家访谈证明，InconLens能显著提高识别失败模式的效率，增强智能体系统的可靠性。
selection_source: fresh_fetch
motivation: LLM智能体的随机性导致相同输入下行为不一，而现有工具难以对多次运行的冗长日志进行有效的跨运行对比分析。
method: 引入“信息节点”作为抽象层来捕捉执行中的关键里程碑，实现多条推理轨迹的语义对齐与可视化交互诊断。
result: 案例研究和专家访谈表明，该系统能帮助开发者快速定位行为分歧点并揭示潜在的失败模式。
conclusion: InconLens为提升LLM智能体系统的稳定性和可靠性提供了高效的视觉诊断工具，填补了跨运行行为分析的空白。
---
