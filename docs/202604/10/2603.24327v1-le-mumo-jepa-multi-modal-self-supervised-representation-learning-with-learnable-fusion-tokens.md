...
tldr: 本研究针对自监督学习多局限于单一模态而忽略传感器互补性的问题，提出了Le MuMo JEPA框架。该框架通过在共享Transformer中引入可学习的融合标记作为潜在瓶颈，实现了RGB图像与LiDAR深度或热成像等模态的统一表示学习。通过剪枝融合策略和SIGReg正则化，该方法在Waymo、nuScenes和FLIR等自动驾驶数据集上展现了卓越的性能与效率平衡，显著提升了下游检测和深度估计任务的效果，同时大幅降低了计算成本。
selection_source: carryover_cache
motivation: 现有的自监督学习方法多关注单一模态，未能充分利用异构传感器（如RGB与LiDAR）之间的互补信息。
method: 扩展LeJEPA架构，利用可学习的融合标记作为模态间的潜在瓶颈，并结合剪枝融合策略与SIGReg正则化进行多模态表示学习。
result: 在Waymo、nuScenes和FLIR数据集上，该方法在检测、深度估计和分割任务中均取得了领先的性能，且计算和内存开销更低。
conclusion: Le MuMo JEPA证明了通过轻量化融合标记实现高效多模态自监督学习的可行性，为自动驾驶等领域提供了高性能且低成本的表征方案。
---
