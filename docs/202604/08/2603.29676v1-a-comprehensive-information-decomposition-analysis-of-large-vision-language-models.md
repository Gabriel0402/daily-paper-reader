...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 大型视觉语言模型中多模态融合的分析
tldr: 本研究针对大型视觉语言模型（LVLM）决策过程不透明的问题，提出了一种基于偏信息分解（PID）的量化分析框架。通过将模型信息分解为冗余、独特和协同三个维度，对26个主流模型在多数据集上进行了跨模型、跨层级及跨训练阶段的深入剖析。研究揭示了任务驱动与知识驱动的差异、不同模型家族的融合策略，以及视觉指令微调在多模态融合中的核心作用，为LVLM的评估与设计提供了量化指导。
selection_source: fresh_fetch
motivation: 旨在探究LVLM的成功是源于真正的多模态融合，还是仅仅依赖于单模态的先验知识。
method: 采用偏信息分解（PID）框架，将模型决策相关信息量化分解为冗余、独特和协同三个组成部分进行分析。
result: 识别出协同驱动与知识驱动两种任务模式，以及融合中心与语言中心两种模型策略，并发现层级处理呈现三阶段模式。
conclusion: 该框架提供了超越准确率的量化评估视角，并证实视觉指令微调是模型习得多模态融合能力的关键阶段。
---
