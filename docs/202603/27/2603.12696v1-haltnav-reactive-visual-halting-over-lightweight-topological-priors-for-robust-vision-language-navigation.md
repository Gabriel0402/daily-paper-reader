...
date: 2026-03-13
pdf: "https://arxiv.org/pdf/2603.12696v1"
tags: ["query:vlm-rl"]
score: 8.0
evidence: 视觉语言导航与开放词汇自主性
tldr: HaltNav是一个分层导航框架，旨在解决视觉语言导航中全局规划与局部环境动态变化之间的矛盾。它利用轻量级的文本拓扑先验（osmAG）进行全局规划，并结合多模态大模型（MLLM）进行局部指令执行。核心贡献是引入了“反应式视觉停止（RVH）”机制，使智能体能识别障碍并动态重规划，同时通过生成式数据合成增强了避障训练，显著提升了导航的鲁棒性。
selection_source: carryover_cache
motivation: 传统的视觉语言导航依赖重型地图且在面对闭门或拥堵等局部环境变化时容易失效，缺乏灵活性。
method: 提出HaltNav框架，利用轻量级osmAG进行全局规划，并通过MLLM驱动的反应式视觉停止机制实现局部障碍感知与动态重规划。
result: 实验证明该框架在无需繁琐指令的情况下优于现有基准，并显著增强了长程导航在动态环境下的成功率。
conclusion: HaltNav通过结合轻量级拓扑先验与反应式视觉感知，为实现稳健、自主的视觉语言导航提供了高效且实用的解决方案。
---

## 摘要
视觉语言导航（VLN）正从僵化的、逐步的指令遵循转向开放词汇、目标导向的自主导航。在没有详尽路由提示的情况下实现这一转变，需要智能体利用结构先验。虽然先前的工作通常假设使用计算量大的 2D/3D 度量地图，但我们转而利用轻量级的、基于文本的 osmAG（OpenStreetMap 区域图），这是一种易于获取和维护的平面图级拓扑表示。然而，仅依靠先验地图进行全局规划在实际部署中是脆弱的，因为局部连通性可能会发生变化（例如，关闭的门或拥挤的通道），从而导致执行时失败。为了弥补这一差距，我们提出了一个分层导航框架 HaltNav，它将 osmAG 的鲁棒全局规划与 VLN 的局部探索和指令对齐能力相结合。我们的方法具有一个基于多模态大语言模型（MLLM）的大脑模块，能够进行高层任务对齐和障碍感知。在 osmAG 的约束下，大脑将全局路径转换为一系列局部执行片段，为 VLN 执行器提供基于先验且以目标为中心的子指令。同时，它通过一种我们称为“反应式视觉停顿”（RVH）的机制来检测局部异常，该机制会中断局部控制循环，通过使相应的拓扑失效来更新 osmAG，并触发重新规划以协调可行的绕行方案。为了高效地训练这种停顿能力，我们引入了一个数据合成流水线，利用生成模型在原本可通行的场景中注入真实的障碍物，从而显著丰富了硬负样本。大量实验表明，我们的分层框架在没有冗长语言指令的情况下优于几种基准方法，并显著提高了在环境变化下长程视觉语言导航的鲁棒性。
