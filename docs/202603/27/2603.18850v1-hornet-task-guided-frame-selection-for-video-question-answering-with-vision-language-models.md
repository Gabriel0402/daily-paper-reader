...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 视觉语言模型在时序推理任务中的策略学习
tldr: "针对视频问答中传统均匀采样无法优化下游回答质量的问题，本文提出HORNet，一种轻量级的任务引导帧选择策略。该方法仅含不到100万参数，利用GRPO强化学习算法训练，使冻结的视觉语言模型能根据问题精准挑选关键帧。实验表明，HORNet在大幅减少输入帧数（最高99%）和处理时间（最高93%）的同时，显著提升了短视频及时间推理任务的回答准确率，且具备良好的跨模型迁移能力。"
