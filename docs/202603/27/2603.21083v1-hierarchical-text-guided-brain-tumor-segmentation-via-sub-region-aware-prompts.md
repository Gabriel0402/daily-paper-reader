...
tags: ["query:mmseg"]
score: 7.0
evidence: 将放射学描述文本与图像结合进行分割的多模态方法
...
motivation: 现有多模态脑肿瘤分割方法通常使用单一全局文本嵌入，忽略了不同子区域独特的临床特征和解剖包含关系。
method: 提出TextCSP框架，结合软级联解码器、子区域感知提示微调及文本语义通道调制器，实现分层且具有子区域针对性的文本引导分割。
result: "在TextBraTS数据集上的实验表明，该方法在Dice和HD95指标上分别比现有最先进方法提升了1.7%和6%。"
conclusion: 通过引入子区域特异性的文本引导和分层级联结构，能够有效捕捉临床描述中的细微特征，显著提升脑肿瘤分割性能。
---
