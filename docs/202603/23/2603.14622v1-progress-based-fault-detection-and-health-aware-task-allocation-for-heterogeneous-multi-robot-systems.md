...
tags: ["query:vlm-rl"]
score: 6.0
evidence: 多机器人任务分配与基于逻辑的故障检测
tldr: 针对异构多机器人系统，本文提出了一种基于任务进度的故障检测模块，并将其与动态任务分配相结合。该方法利用卡尔曼滤波和NIS测试监控任务完成信号，通过健康加权成本和掩码机制优化任务分配。研究贡献在于实现了低计算开销的实时故障检测，并通过凸二次规划在保证任务完成的同时减少了重分配带来的波动，提升了系统的鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决异构机器人团队在面临传感器噪声、执行器偏差或通信中断等故障时，如何实现可靠的故障检测与自适应任务重分配问题。
method: 采用轻量级卡尔曼滤波监控任务进度，并结合健康感知成本函数与L1正则化项，通过凸二次规划实现动态任务分配。
result: 实验表明该系统能及时检测噪声和偏差故障，在保持任务完成率的同时有效限制了任务重分配的频率。
conclusion: 该方案为异构多机器人系统提供了一种计算高效且鲁棒的健康监测与任务调度框架。
---
