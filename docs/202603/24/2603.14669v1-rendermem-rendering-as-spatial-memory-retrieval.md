...
## 摘要
具身推理本质上是视点相关的：什么是可见的、被遮挡的或可达的，关键取决于智能体所处的位置。然而，现有的具身智能体空间记忆系统通常存储多视图观测或以对象为中心的抽象，这使得进行具有显式几何基础的推理变得困难。我们介绍了 RenderMem，这是一个将渲染视为 3D 世界表示与空间推理之间接口的空间记忆框架。RenderMem 不存储固定的观测结果，而是维护一个 3D 场景表示，并通过从查询所隐含的视点渲染场景来生成以查询为条件的视觉证据。这使得具身智能体能够直接从任意视角对视线、可见性和遮挡进行推理。RenderMem 与现有的视觉语言模型完全兼容，且无需对标准架构进行修改。在 AI2-THOR 环境中的实验表明，与之前的记忆基准相比，RenderMem 在视点相关的可见性和遮挡查询方面表现出持续的改进。
