...
title_zh: 多智能体强化学习中干扰感知的 K 步可达通信
...
## 摘要
有效的通信对于解决多智能体强化学习（MARL）中的复杂协作任务至关重要。然而，有限的通信带宽以及动态、复杂的环境拓扑结构在识别高价值通信伙伴方面带来了重大挑战。因此，智能体必须在不确定性下选择合作者，且缺乏关于哪些伙伴能够提供任务关键信息的先验知识。为此，我们提出了干扰感知 K 步可达通信（IA-KRC），这是一个通过两个核心组件增强协作的新型框架：（1）一种将消息传递限制在物理可达邻居范围内的 K 步可达性协议；（2）一个通过最小化干扰并最大化效用来优化伙伴选择的干扰预测模块。与现有方法相比，尽管存在环境干扰，IA-KRC 仍能实现显著更持久且高效的协作。全面评估证实，与最先进的基准方法相比，IA-KRC 取得了更优越的性能，同时在复杂拓扑和高度动态的多智能体场景中展示了增强的鲁棒性和可扩展性。
