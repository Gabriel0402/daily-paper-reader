...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 具身视觉-语言-动作 (VLA) 模型的动态量化
tldr: "针对具身智能中VLA模型推理开销大且静态量化忽略时序动态敏感性的问题，本文提出DyQ-VLA动态量化框架。该框架利用运动学代理实时感知任务阶段的敏感度，并动态调整量化位宽。实验表明，DyQ-VLA在仅占用30.9%显存的情况下，保持了99.5%的性能，并显著提升了仿真与实测的推理速度，为边缘端部署提供了高效支持。"
selection_source: carryover_cache
motivation: 传统的静态量化方法忽略了VLA模型在不同动作阶段对精度的差异化需求，导致推理效率与动作准确性难以兼得。
method: 引入敏感度感知切换策略和运动学引导模块，利用实时运动学指标作为代理来指导动态位宽分配。
result: "该方法将显存占用降低至30.9%，同时维持了99.5%的性能，并在真实世界中实现了高达1.43倍的推理加速。"
conclusion: DyQ-VLA证明了通过感知时序动态性进行动态量化是提升具身智能模型部署效率的有效途径。
---
