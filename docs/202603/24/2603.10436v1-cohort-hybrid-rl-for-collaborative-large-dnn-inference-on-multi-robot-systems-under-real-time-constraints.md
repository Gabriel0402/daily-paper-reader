...
title_zh: COHORT：实时约束下多机器人系统协作大型 DNN 推理的混合强化学习
...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 多机器人系统上协作多模态深度神经网络推理的混合强化学习
...
motivation: 资源受限的机器人集群在执行大型DNN推理时，面临带宽、延迟和电池寿命的严峻挑战，且往往缺乏外部服务器支持。
...
## 摘要
大型深度神经网络（DNN），特别是基于 Transformer 和多模态架构的网络，计算需求极高，且难以部署在野外机器人等资源受限的边缘平台上。在任务关键型场景（如灾难响应）中，这些挑战更加严峻，机器人必须在带宽、延迟和电池寿命的严格约束下进行协作，且通常缺乏基础设施或服务器支持。为了解决这些局限性，我们提出了 COHORT，这是一个基于机器人操作系统（ROS）构建的多机器人系统协作 DNN 推理和任务执行框架。COHORT 采用混合离线-在线强化学习（RL）策略，在机器人之间动态调度和分配 DNN 模块的执行。我们的主要贡献包括三个方面：(a) 结合优势加权回归（AWR）的离线强化学习策略学习，利用分布式机器人异构 DNN 工作负载的基于拍卖的任务分配数据进行训练；(b) 通过多智能体 PPO（MAPPO）进行在线策略自适应，由离线策略初始化并进行实时微调；(c) 在 CLIP 和 SAM 等视觉语言模型（VLM）推理任务上对 COHORT 进行全面评估，分析其在机器人/工作负载增加时的可扩展性以及鲁棒性。我们将 COHORT 与遗传算法和多个强化学习基准进行了对比。实验结果表明，COHORT 将电池消耗降低了 15.4%，GPU 利用率提高了 51.67%，同时满足帧率和截止时间约束的成功率是对比方案的 2.55 倍。
