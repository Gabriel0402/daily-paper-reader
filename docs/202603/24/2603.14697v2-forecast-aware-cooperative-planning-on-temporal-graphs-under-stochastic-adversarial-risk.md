...
tags: ["query:vlm-rl"]
score: 7.0
evidence: 基于时序图和风险预测的多机器人协作规划
tldr: 针对多机器人任务中动态变化的对抗性风险，本文提出了一种基于预测的协作规划框架。该框架通过将对手动态建模为一阶马尔可夫过程，生成时变风险预测，并据此进行前瞻性的支援分配与联合路径规划。研究解决了现有协作方法在静态风险假设下的局限性，显著降低了团队任务成本，在动态环境中表现接近理想规划器。
selection_source: fresh_fetch
motivation: 现有的多机器人协作框架通常假设风险环境是静态的，无法应对随时间变化的对抗性风险或环境危害。
method: 将对手动态建模为一阶马尔可夫过程以生成时变风险预测，并结合前瞻性支援分配与联合路径规划。
result: 实验表明，该方法在降低团队总预期成本方面显著优于非前瞻性基准，且性能接近已知未来信息的理想规划器。
conclusion: 整合随机风险预测与前瞻性协作规划能有效提升多机器人在动态风险环境中的任务效率与安全性。
---

## 摘要
协作式多机器人任务通常要求机器人团队穿越环境，由于对手巡逻或具有随机动态的移动危险，穿越风险会随之演变。虽然支持协调（即机器人协助队友穿越危险区域）可以显著降低任务成本，但其有效性取决于团队预测未来风险的能力。现有的基于支持的框架假设风险景观是静态的，因此无法考虑风险演变中可预测的时间趋势。我们提出了一种预测感知协作规划框架，该框架将随机风险预测与时序图上的前瞻性支持分配相结合。通过将对手动态建模为图边上的一阶马尔可夫停留-移动过程，我们将生成的边占用概率在时间上向前传播，以生成带时间索引的边风险预测。这些预测指导将支持位置主动分配到预测的风险边，以实现有效的支持协调，同时也为联合机器人路径规划提供信息。实验结果表明，与非前瞻性基准相比，我们的方法始终能降低团队的总预期成本，性能接近理想规划器。
