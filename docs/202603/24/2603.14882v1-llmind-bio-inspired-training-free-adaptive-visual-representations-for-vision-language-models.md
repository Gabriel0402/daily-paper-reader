...
tags: ["query:mmseg"]
score: 6.0
evidence: 视觉语言模型的仿生自适应视觉表示
tldr: 针对视觉语言模型（VLM）在处理视觉输入时空间保真度均匀且效率低下的问题，本文提出 LLMind 框架。该框架受人类视觉启发，采用无需训练的自适应表示方法，通过仿生自适应采样策略（BASS）和闭环语义反馈（CSF）模拟中心凹编码。LLMind 在极低像素预算下显著提升了 VLM 的性能，且具有轻量级、即插即用的特点，无需修改现有模型架构。
selection_source: fresh_fetch
motivation: 现有的视觉语言模型对所有视觉区域分配相同的精度，缺乏人类视觉那种自适应、选择性和资源高效的特性。
...
conclusion: LLMind 证明了仿生自适应视觉表示在提升 VLM 效率和性能方面的巨大潜力，且无需额外训练即可与现有模型无缝集成。
---
