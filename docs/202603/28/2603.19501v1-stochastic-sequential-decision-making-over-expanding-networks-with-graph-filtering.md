...
tags: ["query:gt-mai"]
score: 6.0
evidence: 用于序列决策的多智能体系统建模
tldr: 针对传统图滤波方法难以处理动态扩张网络的问题，本文提出一种基于随机序列决策的图滤波框架。该方法将滤波器偏移建模为多智能体系统，利用强化学习捕捉网络扩张的长期影响。通过上下文感知图神经网络参数化策略，实现了在冷启动推荐和疫情预测等任务中优于传统批量和在线滤波的效果，为动态网络数据处理提供了新视角。
selection_source: fresh_fetch
motivation: 现有的图滤波方法多针对固定图，难以应对节点持续增加且模式未知的动态扩张网络及其带来的不确定性。
method: 将滤波器建模为多智能体系统，并结合上下文感知图神经网络，通过多智能体强化学习训练自适应扩张网络的滤波策略。
result: 在合成数据、冷启动推荐及疫情预测等真实数据集上的实验表明，该序列决策方法在处理动态网络时优于传统的批量和在线滤波方案。
conclusion: 引入序列决策视角并结合强化学习，能有效提升图滤波器在动态扩张网络环境下的适应性和长期预测性能。
---

## 摘要
图滤波器利用拓扑信息来处理网络化数据。现有方法主要研究固定图，忽略了图通常会随着节点以未知模式不断加入而扩张。后者需要开发能够考虑演化和不确定性的基于滤波器的决策范式。现有方法依赖于预先设计的滤波器或在线学习，局限于仅考虑过去或当前信息的短视视角。为了考虑未来的影响，我们提出了一种用于过滤网络化数据的随机序列决策框架，该框架采用一种使滤波适应扩张图的策略。通过将滤波器移位（filter shifts）表示为智能体，我们将滤波器建模为多智能体系统，并按照多智能体强化学习训练策略。这考虑了长期奖励，并通过序列决策捕捉扩张动态。此外，我们开发了一种上下文感知图神经网络来参数化该策略，该网络根据图和智能体的信息来调整滤波器参数。在从冷启动推荐到 COVID 预测的合成和真实数据集上的实验表明，与批量和在线滤波替代方案相比，使用序列决策视角具有显著优势。
