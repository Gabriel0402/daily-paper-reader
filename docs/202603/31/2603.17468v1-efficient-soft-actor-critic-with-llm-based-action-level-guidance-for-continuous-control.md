...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 基于LLM的强化学习动作级引导
tldr: 针对强化学习在广阔状态动作空间中探索效率低的问题，本文提出GuidedSAC算法。该方法利用大语言模型（LLM）作为智能监督者，通过分析轨迹状态和视觉回放提供动作级引导，干预Soft Actor-Critic（SAC）的探索过程。理论证明其在保留SAC收敛性的同时提升了收敛速度。实验表明，GuidedSAC在MuJoCo等任务中显著优于标准SAC及RND、ICM等主流探索增强算法，展现了极高的采样效率。
selection_source: carryover_cache
motivation: 旨在解决传统强化学习算法在复杂连续控制任务中因探索效率低下而导致的采样效率低和性能受限问题。
method: 提出一种将LLM作为动作级监督者的框架，通过分析历史轨迹对SAC算法进行针对性干预和引导。
result: 实验证明GuidedSAC在多种离散和连续控制任务中的表现均优于标准SAC以及RND、ICM等先进的探索增强变体。
conclusion: GuidedSAC通过引入LLM引导机制，在保证收敛性的前提下显著提升了强化学习的探索效率和最终任务性能。
---
