...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 具有端到端感知和规划的视觉语言导航
tldr: 针对视觉语言导航（VLN）中智能体缺乏全面场景理解的问题，本文提出了P³Nav框架。该框架将感知、预测和规划集成到统一的端到端流程中，通过提取物体级和地图级特征增强感知，并预测未来航路点及语义地图线索，从而实现前瞻性规划。实验证明，该方法在REVERIE、R2R-CE和RxR-CE等多个基准测试上达到了SOTA性能。
selection_source: carryover_cache
motivation: 现有VLN方法过度依赖视觉-文本对齐，缺乏对环境的深度感知和对未来状态的预测能力，限制了复杂场景下的规划效率。
method: 提出一种端到端框架，通过整合物体与地图级感知、未来航路点预测以及语义地图预报，增强智能体的场景理解与前瞻性规划能力。
result: 该方法在REVERIE、R2R-CE和RxR-CE等多个主流视觉语言导航基准测试中均刷新了最先进的性能记录。
conclusion: 通过在导航流程中引入显式的感知与预测环节，可以有效提升智能体在动态或复杂环境中的决策质量和任务成功率。
---

## 摘要
在视觉语言导航（VLN）中，智能体需要利用其视觉观测，根据语言指令规划通往目标的路径。因此，主流的 VLN 方法主要侧重于通过视觉-文本对齐来构建强大的规划器。然而，这些方法往往忽略了在规划前进行全面场景理解的必要性，导致智能体的感知或预测能力不足。为此，我们提出了 P$^{3}$Nav，这是一种新颖的端到端框架，在统一的流水线中集成了感知、预测和规划，以增强 VLN 智能体的场景理解能力并提高导航成功率。具体而言，P$^{3}$Nav 通过从物体级和地图级视角提取互补线索来增强感知。随后，P$^{3}$Nav 预测路标点以建模智能体潜在的未来状态，赋予智能体在导航过程中对候选位置的内在感知。以这些未来路标点为条件，P$^{3}$Nav 进一步预测语义地图线索，从而实现主动规划并减少对纯历史上下文的严格依赖。综合这些感知和预测线索，一个整体规划模块最终执行 VLN 任务。大量实验表明，我们的 P$^{3}$Nav 在 REVERIE、R2R-CE 和 RxR-CE 基准测试中取得了新的最先进性能。
