...
tags: ["query:mmseg"]
score: 7.0
evidence: 使用视觉基础模型 (DINOv3) 进行语义分割任务
...
method: 提出包含分层特征调制（HFM）、超图合理性评估（HPE）和结构异常剪枝（SAP）的SHAPE框架，从特征和结构层面优化模型。
result: "在心脏和腹部跨模态分割任务中，SHAPE显著优于现有方法，Dice评分最高达到90.08%。"
conclusion: 通过引入全局解剖合理性评估，SHAPE有效解决了UDA中的结构失真问题，为医学图像跨模态分割提供了更稳健的方案。
---
