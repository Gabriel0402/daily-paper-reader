...
tldr: 本文针对跨域少样本学习（CDFSL）中CLIP模型难以捕捉目标域细粒度视觉特征的问题，提出了CC-CDFSL方法。研究发现领域差异和数据稀缺加剧了局部视觉特征与文本语义的失配。该方法利用循环一致性原理，在视觉和文本特征间建立互译约束，并引入语义锚点机制过滤噪声。实验证明，该方法不仅显著提升了模型在下游任务中的分类性能，还通过可视化补丁增强了模型决策的可解释性。
selection_source: fresh_fetch
motivation: 现有的CLIP模型在跨域少样本场景下难以聚焦于细粒度的局部视觉线索，存在严重的局部特征与语义失配问题。
method: 提出CC-CDFSL方法，利用循环一致性进行视觉-文本特征互译约束，并结合语义锚点机制优化特征映射。
result: 实验表明该方法在多个基准测试上均取得了最先进的性能，并有效提升了局部对齐质量和模型可解释性。
conclusion: 通过自监督的循环一致性约束，可以有效纠正跨域场景下的局部对齐偏差，是提升CDFSL性能和可解释性的有效途径。
---
