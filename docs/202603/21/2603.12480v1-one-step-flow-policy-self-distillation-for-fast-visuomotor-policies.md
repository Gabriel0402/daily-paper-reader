...
result: 在56项任务中，单步 OFP 的表现优于100步扩散模型，推理速度提升超过100倍，并成功集成于 π0.5 模型。
conclusion: OFP 证明了通过自蒸馏实现高性能、低延迟的单步视觉运动策略是可行且高效的，具有极强的实用性。
---

## 摘要
生成式流模型和扩散模型提供了高精度机器人策略所需的连续、多模态动作分布。然而，它们对迭代采样的依赖引入了严重的推理延迟，降低了控制频率，并损害了时间敏感型操作任务中的性能。为了解决这一问题，我们提出了“一步流策略”（One-Step Flow Policy, OFP），这是一个从零开始的自蒸馏框架，无需预训练教师模型即可实现高保真、单步动作生成。OFP 结合了自一致性损失（用于强制跨时间间隔的连贯传输）和自引导正则化（用于使预测向高密度专家模式收敛）。此外，热启动机制利用动作的时间相关性来最小化生成传输距离。在 56 个不同的模拟操作任务中的评估表明，单步 OFP 达到了最先进的结果，在动作生成速度提高 100 倍以上的同时，性能优于 100 步扩散和流策略。我们进一步将 OFP 集成到 RoboTwin 2.0 上的 π₀.₅ 模型中，单步 OFP 的表现超过了原始的 10 步策略。这些结果证明了 OFP 是一种用于高精度、低延迟机器人控制的实用且可扩展的解决方案。
