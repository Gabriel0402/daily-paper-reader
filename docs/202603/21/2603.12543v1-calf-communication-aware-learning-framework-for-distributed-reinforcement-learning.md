...
method: 提出CALF框架，在仿真训练阶段显式建模并引入延迟、抖动和丢包等真实网络约束。
result: 实验表明，与忽略网络因素的基准相比，CALF显著减少了部署时的性能损失，并在异构硬件上验证了其鲁棒性。
conclusion: 网络通信条件是分布式强化学习sim-to-real迁移中不可忽视的重要维度，必须在训练中予以显式考虑。
---

## 摘要
分布式强化学习策略在边缘设备和云服务器之间部署时，面临网络延迟、抖动和丢包等问题。标准的强化学习训练假设交互为零延迟，这导致在现实网络条件下性能严重下降。我们引入了 CALF（通信感知学习框架），该框架在模拟过程中基于现实网络模型进行策略训练。系统实验表明，与不感知网络的基准方法相比，网络感知训练显著缩小了部署性能差距。在异构硬件上的分布式策略部署验证了，在训练期间显式建模通信约束能够实现稳健的现实世界执行。这些发现确立了网络条件是类 Wi-Fi 分布式部署中“模拟到现实”（sim-to-real）迁移的一个主要维度，是对物理和视觉领域随机化方法的补充。
