...
tags: ["query:vlm-rl"]
score: 7.0
evidence: 大语言模型智能体协调与委派协议
tldr: 本研究针对大语言模型代理在协作中因信息不对称导致的脆弱性，提出了一种任务感知的委派信号层。通过对 Chatbot Arena 数据进行语义聚类，构建了任务分类体系，并推导出能力概况和协作风险提示。该框架将不透明的委派机制转变为可见、可协商且可审计的协作决策过程，支持自适应路由和显式理由披露，显著提升了人机协作的透明度与可靠性。
selection_source: fresh_fetch
motivation: 解决人类用户与 LLM 代理协作时因缺乏任务特定可靠性线索和不确定性反馈而导致的信息不对称问题。
method: 利用 Chatbot Arena 的成对比较数据进行语义聚类以诱导任务分类，并据此生成能力概况和协调风险提示。
result: 实验证明任务类型具有可操作的结构，聚类特征有效提升了胜率预测准确性并降低了任务难度预测误差。
conclusion: 该框架为自适应人机协作提供了一个基于相互感知和共同问责的原则性设计空间，使委派过程更加透明和可控。
---

## 摘要
大语言模型（LLM）智能体越来越多地以对话协作者的身份出现，但由于信息不对称，人机协作仍然十分脆弱：用户缺乏特定任务的可靠性线索，而智能体很少呈现校准后的不确定性或推理依据。我们提出了一种任务感知的协作信号层，将离线偏好评估转化为面向用户的在线委派原语。利用 Chatbot Arena 的成对比较数据，我们通过语义聚类归纳出一种可解释的任务分类法，进而推导出：(i) 作为任务条件胜率图的“能力概况”（Capability Profiles），以及 (ii) 作为任务条件分歧（平局率）先验的“协作风险线索”（Coordination-Risk Cues）。这些信号驱动了一个闭环委派协议，该协议支持共识验证、自适应路由（主智能体 vs. 主智能体+审计者）、显式推理披露以及隐私保护的问责日志。两项预测性探针实验验证了任务类型具有可操作的结构：在分层五折交叉验证下，聚类特征提高了胜者预测准确率，并降低了难度预测误差。总的来说，我们的框架将委派从一种不透明的系统默认行为重构为一种可见、可协商且可审计的协作决策，为基于相互感知和共同问责的自适应人机协作提供了一个有原则的设计空间。
