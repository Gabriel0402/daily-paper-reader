...
## 摘要
本文开发了一个控制论框架，用于分析嵌入在反馈控制回路中的智能体系统。在此类系统中，AI 智能体可以在运行过程中调整控制器参数、选择控制策略、调用工具、重新配置决策架构或修改控制目标。我们通过将代理性（agency）解释为对控制架构的分层决策权，将这些能力形式化。文中引入了一种统一的动力学表示，将记忆、学习、工具激活、交互信号和目标描述符整合到单一的闭环结构中。基于这一表示，我们定义了代理性的五个层级，涵盖从反应式规则控制到控制目标与控制器架构综合的各个阶段。该框架在非线性和线性设置中均有呈现，允许使用标准的控制论构造（如反馈增益、切换信号、参数自适应律和二次代价函数）来解释智能体行为。分析表明，增加代理性会引入动力学机制，包括时变自适应、内生切换、决策诱导延迟以及控制流水线的结构重构。这一视角为分析 AI 赋能的控制系统的稳定性、安全性和性能提供了数学基础。
