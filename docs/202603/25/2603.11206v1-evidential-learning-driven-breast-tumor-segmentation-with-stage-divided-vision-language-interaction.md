...
## 摘要
乳腺癌是全球女性最常见的死因之一，每年导致数百万人死亡。磁共振成像（MRI）可以提供多种序列来表征肿瘤形态和内部模式，已成为乳腺肿瘤检测和诊断的有效工具。然而，由于癌症区域与正常区域之间对比度低以及边界模糊的挑战，以往基于深度学习的肿瘤分割方法在准确定位肿瘤轮廓方面存在局限性。利用文本提示信息有望通过描绘分割区域来改善肿瘤分割效果。受此启发，我们提出了一种结合分阶段视觉-语言交互和证据学习的文本引导乳腺肿瘤分割模型（TextBCS）。具体而言，所提出的分阶段视觉-语言交互促进了下采样每个阶段视觉特征与文本特征之间的信息互通，进一步发挥文本提示的优势，辅助在低对比度场景下定位病变区域。此外，采用证据学习来量化模型针对模糊边界的分割不确定性。它利用变分狄利克雷（variational Dirichlet）来表征分割概率的分布，从而解决边界的分割不确定性。大量实验验证了我们的 TextBCS 优于其他分割网络，在公开数据集上展示了最佳的乳腺肿瘤分割性能。
