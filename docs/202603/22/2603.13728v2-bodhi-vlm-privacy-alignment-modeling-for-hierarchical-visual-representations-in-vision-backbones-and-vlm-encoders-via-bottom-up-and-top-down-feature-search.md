...
## 摘要
保护隐私的学习系统通常在层级视觉表示中注入噪声；其核心挑战在于如何以一种可解释且适用于视觉骨干网络和视觉语言模型（VLM）的方式，对这些扰动与声明的隐私预算之间的对齐进行建模。我们提出了 Bodhi VLM，这是一个针对层级神经表示的隐私对齐建模框架：它 (1) 通过基于 NCP 和 MDAV 的聚类将敏感概念与逐层分组
