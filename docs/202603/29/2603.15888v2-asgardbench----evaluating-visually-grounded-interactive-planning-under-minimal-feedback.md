...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 视觉落地的交互式规划与动作序列生成
tldr: AsgardBench是一个专注于评估视觉驱动交互式规划能力的基准测试，旨在测试模型在极简反馈下根据视觉观察调整高层动作序列的能力。该基准包含12类任务，通过系统性改变物体状态和场景配置，强调执行过程中的条件分支和计划修复。研究发现，领先的视觉语言模型在依赖视觉进行动态规划时仍存在显著短板。
selection_source: carryover_cache
motivation: 现有具身智能基准常将高层推理与底层控制混淆，且提供过多反馈，无法有效隔离评估模型基于视觉观察进行交互式规划的能力。
method: 设计了108个受控任务实例，限制输入为图像、动作历史和简单的成败信号，要求模型在模拟器中根据视觉反馈实时修正计划。
result: 评估显示主流视觉语言模型在失去视觉输入后性能骤降，暴露出其在视觉定位和环境状态跟踪方面的严重缺陷。
conclusion: AsgardBench 证明了当前模型在利用视觉信息进行动态计划调整方面仍面临挑战，为具身智能的交互式规划研究提供了精准的评估工具。
---

## 摘要
通过 AsgardBench，我们旨在评估视觉对齐的高层动作序列生成和交互式规划，特别关注执行过程中基于视觉观测的规划调整，而非导航或底层操作。在具身智能（Embodied AI）基准测试领域，AsgardBench 针对的是交互式规划这一能力类别。相比离线高层规划，交互式规划更为复杂，因为它要求智能体根据环境反馈修正规划，但又与底层执行保持独立。不同于以往将推理与导航混为一谈，或提供丰富的纠错反馈以替代感知的具身智能基准测试，AsgardBench 将智能体的输入限制为图像、动作历史和轻量级的成功/失败信号，从而在没有底层控制噪声的可控模拟器中隔离出交互式规划。该基准测试包含涵盖 12 种任务类型的 108 个任务实例，每个实例通过物体状态、放置位置和场景配置进行系统性变化。这些受控变量创建了条件分支，使得同一条指令根据智能体的观测可能需要不同的动作序列，强调了执行过程中的条件分支和规划修复。我们对领先视觉语言模型的评估表明，在没有视觉输入的情况下，性能会急剧下降，揭示了视觉对齐和状态跟踪方面的弱点，这些弱点最终损害了交互式规划。我们的基准测试聚焦于一个更具体的问题：当情况不符合预期时，模型是否真的能利用其所见信息来调整规划？
