...
tags: ["query:vlm-rl"]
score: 8.0
evidence: 用于LLM推理的多智能体系统，包含专门的智能体
tldr: 针对大语言模型在复杂多步推理中的局限性，本文受人类认知中的全局工作空间理论启发，提出了脑启发图多智能体系统（BIGMAS）。该系统将专用智能体组织为动态有向图节点，通过中心化共享工作空间进行协作。通过GraphDesigner构建任务拓扑和Orchestrator进行全局调度，BIGMAS在Game24等复杂任务上显著提升了推理性能，证明了多智能体架构设计与模型级推理增强具有互补性。
selection_source: carryover_cache
motivation: 现有的LLM和LRM在处理极高复杂度的多步推理任务时仍面临准确率崩溃，单纯依靠模型规模扩展难以解决该问题。
method: 提出BIGMAS系统，利用动态图拓扑组织智能体，并基于全局工作空间理论通过中心化协调器实现跨智能体的信息共享与路由。
result: 在Game24、Six Fives和伦敦塔等任务上的实验表明，BIGMAS在多种前沿模型上均优于ReAct和思维树等现有基准。
conclusion: 多智能体架构设计能为大模型推理提供与模型自身增强相正交的性能提升，是解决复杂推理问题的有效途径。
---

## 摘要
大语言模型（LLMs）在广泛的语言任务中展现了卓越的能力，但复杂的多步推理仍然是一个根本性的挑战。虽然配备了扩展思维链机制的大型推理模型（LRMs）表现出优于标准 LLMs 的性能，但在足够复杂的任务上，这两类模型仍会遭遇准确率崩塌，这表明仅靠扩展模型层面的推理是不够的。受人类认知全局工作空间理论的启发，我们提出了类脑启发式图多智能体系统（BIGMAS）。在该系统中，专门的 LLM 智能体被组织为动态构建的有向图中的节点，并完全通过一个集中的共享工作空间进行协作。一个问题自适应的图设计器（GraphDesigner）构建特定任务的智能体拓扑结构，而全局编排器（Orchestrator）利用完整的共享状态进行路由决策，克服了反应式方法中的局部视图瓶颈。在 Game24、Six Fives 和 Tower of London 任务上，针对六种前沿 LLMs 的实验表明，BIGMAS 持续提升了标准 LLMs 和 LRMs 的推理性能，优于包括 ReAct 和思维树（Tree of Thoughts）在内的现有多智能体基准，证明了多智能体架构设计提供了与模型层面推理增强相正交的互补增益。
