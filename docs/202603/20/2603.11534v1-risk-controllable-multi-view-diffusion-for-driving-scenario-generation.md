...
tags: ["query:mmseg"]
score: 6.0
evidence: 自动驾驶的多视图驾驶场景生成
tldr: 针对自动驾驶中长尾风险场景稀缺且难以生成的问题，本文提出RiskMV-DPO框架。该框架通过物理风险建模合成动态轨迹作为几何锚点，并结合多视图扩散模型与区域感知直接偏好优化（RA-DPO）策略，实现了高保真、时空一致且风险等级可控的驾驶场景生成，为自动驾驶系统的安全评估提供了高效的合成工具。
selection_source: fresh_fetch
motivation: 现有生成方法难以精确控制风险等级，且在生成多视图驾驶场景时难以维持几何一致性与时空连贯性。
method: 集成物理风险建模以生成动态轨迹锚点，并引入几何外观对齐模块与带运动感知掩码的区域感知直接偏好优化策略。
result: 在nuScenes数据集上，该方法将3D检测mAP从18.17提升至30.50，并将FID降低至15.70，展现了卓越的视觉质量与生成多样性。
conclusion: 该研究成功将世界模型从被动环境预测转变为主动、风险可控的合成工具，为具身智能的安全导向开发提供了可扩展的方案。
---
