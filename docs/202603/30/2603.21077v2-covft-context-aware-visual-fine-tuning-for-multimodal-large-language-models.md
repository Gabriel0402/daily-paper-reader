...
tldr: 针对多模态大模型中视觉编码器微调不稳定的问题，本文揭示了视觉偏好冲突是导致性能波动的核心原因。为此提出了CoVFT框架，通过上下文向量提取和上下文混合专家模块，将多模态上下文显式融入视觉适配过程。实验证明，CoVFT在12个基准测试中实现了SOTA性能与极高的稳定性，甚至使7B模型在平均表现上超越了13B模型，挖掘了视觉编码器优化的巨大潜力。
selection_source: fresh_fetch
motivation: 现有视觉微调方法因忽略多模态上下文导致的视觉偏好冲突，难以在不同任务中稳定优于冻结基座的基准。
method: 提出CoVFT框架，利用上下文向量提取（CVE）和上下文混合专家（CoMoE）模块实现上下文感知的视觉参数更新。
result: 在12个多模态基准测试中取得领先性能，且微调后的7B模型表现优于同系列的13B模型。
conclusion: 显式引入上下文感知的视觉微调能有效解决优化冲突，是提升多模态大模型性能的关键路径。
---
