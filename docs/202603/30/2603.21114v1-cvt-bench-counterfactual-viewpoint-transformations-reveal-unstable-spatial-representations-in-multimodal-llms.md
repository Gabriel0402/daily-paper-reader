...
tags: ["query:mmseg"]
score: 6.0
evidence: 评估多模态大语言模型中的空间表示
tldr: 本研究针对多模态大模型（MLLM）在空间推理中的稳定性问题，提出了CVT-Bench基准测试。该基准通过模拟相机轨道变换的虚拟视角改变，评估模型在不重新渲染图像情况下的关系一致性。实验发现，尽管模型在单视角下表现优异，但在反事实视角变换下表现出显著的不稳定性。研究还表明，增加输入表示的结构化程度（如场景图）能有效提升模型的空间推理鲁棒性。
selection_source: fresh_fetch
motivation: 旨在探究多模态大模型在单视角空间推理表现良好的背后，是否具备在反事实视角变换下保持稳定空间状态表示的能力。
method: 引入CVT-Bench基准，通过100个合成场景和6000个查询，在不重绘图像的情况下评估模型在视角变换下的关系一致性、循环一致性及稳定性。
result: 实验显示主流MLLM在反事实视角变换下性能系统性下降，频繁违反循环一致性，且关系稳定性随变换次数增加而迅速衰减。
conclusion: 单视角空间准确率高估了模型空间表示的鲁棒性，而结构化的表示形式对于提升反事实空间推理的稳定性至关重要。
---
